 ...
 */
async function getDashboardMetrics(userId, role) {
    const where = role === 'ADMIN' ? {} : { userId };
    const [incomeAgg, expenseAgg, recordCount] = await Promise.all([
        prisma_1.default.financialRecord.aggregate({
    ...
    const topCategoriesRaw = await prisma_1.default.financialRecord.groupBy({
        by: ['category'],
        where: { ...where, type: 'EXPENSE' },
        _sum: { amount: true },
     ...
     */
    let monthlyTrendsRaw;
    if (role === 'ADMIN') {
        monthlyTrendsRaw = await prisma_1.default.$queryRaw `
      SELECT 
